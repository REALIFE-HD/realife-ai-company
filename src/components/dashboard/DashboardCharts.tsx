          ...
          return (
            <div key={d.name} className="flex flex-1 flex-col items-center gap-2">
              <div className="relative flex h-full w-full items-end">
                <div
                  className="w-full rounded-t-sm bg-gradient-to-t from-teal-500 to-teal-400 transition-all"
                  style={{ height: `${h}%` }}
                  aria-label={`${d.name} ${d.value} タスク`}
                />
              </div>
              <span className="font-mono text-[10px] text-slate-500">{d.value}</span>
            </div>
          );
        })}
      </div>
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900">部門ステータス分布</h3>
          <span className="font-mono text-[10px] uppercase tracking-wider text-slate-400">12 部門</span>
        </div>
        <div className="mt-6">
          <Donut />
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-900">部門別タスク負荷</h3>
          <span className="font-mono text-[10px] uppercase tracking-wider text-slate-400">Top 8</span>
        </div>
        <div className="mt-6">
          <Bars />
        </div>
