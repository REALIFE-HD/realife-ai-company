...
export function SearchHistoryDropdown({
  ...
  if (history.length === 0) {
    return (
      <div className="absolute left-0 right-0 top-full z-30 mt-1 rounded-md border border-border bg-card p-3 text-[12px] text-muted-foreground shadow-lg">
        検索履歴はまだありません
      </div>
    );
  }
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-1.5">
        <span className="flex min-w-0 items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
          ...
          {typeof limit === "number" && (
            <span
              className="rounded-sm bg-muted px-1.5 py-px font-mono text-[10px] font-medium tracking-normal text-muted-foreground"
          ...
          <span
            className={`rounded-sm px-1.5 py-px font-mono text-[10px] font-medium tracking-normal ${
              byteWarn ? "bg-amber-100 text-amber-800" : "bg-muted text-muted-foreground"
            }`}
          ...
          <span
            ...
            className="inline-flex h-3.5 w-3.5 cursor-help items-center justify-center text-muted-foreground hover:text-muted-foreground"
          >
      ...
      {typeof limit === "number" && history.length >= limit && (
        <div className="border-b border-border bg-amber-50/60 px-3 py-1 text-[10.5px] text-amber-800">
            ...
            >
              <button
                ...
                className={`flex flex-1 items-center gap-2 px-3 py-1.5 text-left text-[13px] ${
                  isActive ? "text-blue-900" : "text-muted-foreground hover:bg-muted"
                }`}
              >
                <Clock
                  className={`h-3.5 w-3.5 ${isActive ? "text-blue-500" : "text-muted-foreground"}`}
                  aria-hidden="true"
                />
              ...
              <button
                ...
                className="mr-1 inline-flex h-6 w-6 items-center justify-center rounded text-muted-foreground opacity-0 transition-opacity hover:bg-muted hover:text-muted-foreground group-hover:opacity-100"
              >
                <X className="h-3 w-3" />
              </button>
            </li>
